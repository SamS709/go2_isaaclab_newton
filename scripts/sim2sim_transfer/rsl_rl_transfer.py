...
import go2_isaaclab_newton.tasks  # noqa: F401

def get_joint_mappings_go2(args_cli, action_space_dim):
    ...
    obs_map = (
        ...
        + [6, 7, 8]
        + [9]
        + [i + 10 + num_joints * 0 for i in source_to_target]
        + [i + 10 + num_joints * 1 for i in source_to_target]
        + [i + 10 + num_joints * 2 for i in source_to_target]
        + [46, 47, 48, 49]
    )
...
def main():
    ...
    # Check if the checkpoint is a TorchScript model
    loaded = torch.load(resume_path, map_location=agent_cfg.device)
    print(f"[INFO]: Loaded checkpoint type: {type(loaded)}")
    if isinstance(loaded, dict):
        print(f"[INFO]: Checkpoint keys: {list(loaded.keys())}")
    elif isinstance(loaded, torch.jit.ScriptModule):
        print(f"[INFO]: Loaded TorchScript model")
    else:
        print(f"[INFO]: Unknown checkpoint type")
    
    if isinstance(loaded, torch.jit.ScriptModule):
        # If it's a scripted model, set it directly
        ppo_runner.alg.policy = loaded
    else:
    ...
    _, target_to_source, obs_map = get_joint_mappings_go2(args_cli, env.action_space.shape[1])
